...
export const addTenancy = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const expires = Date.now() + 14 * 24 * 60 * 60 * 1000; // 14 days expiry
    ...
    const existingTenancy = await ctx.db
      ...
      .filter((q) =>
        q.and(
          ...
          q.or(
            q.eq(q.field("status"), TenancyStatus.INVITED),
            q.eq(q.field("status"), TenancyStatus.PENDING_TENANT_RESPONSE),
            q.eq(q.field("status"), TenancyStatus.PENDING_CONFIRMATION),
            q.eq(q.field("status"), TenancyStatus.ACTIVE),
          ),
        ),
      )
      .first();
    ...
    // Create the tenancy record for landlord-initiated flow
    const tenancyId = await ctx.db.insert("tenancies", {
      ...
      // Landlord-initiated flow defaults
      freeReviewEligible: false, // Landlord-initiated = not free
      landlordReviewable: true, // Default to mutual review for landlord-initiated
      mutualReviewAgreed: true, // Landlord-initiated defaults to mutual
      tenantVerified: false,
      landlordVerified: true, // Landlord is already verified since they initiated
      addressVerified: true, // Property address is verified by landlord
