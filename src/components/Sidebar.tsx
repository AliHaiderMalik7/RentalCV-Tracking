...
import { api } from "../../convex/_generated/api";
  ...
  const Sidebar = ({ role, activeTab, onTabChange }: SidebarProps) => {
    ...
    return (
      <div className="w-72 h-screen bg-[#075985] fixed left-0 top-0 flex flex-col border-r border-[#0a7ba8]/30">
        {/* Premium Logo Header */}
        <div className="px-6 py-7 bg-white/95 backdrop-blur-sm border-b border-[#0a7ba8]/10">
          <div className="flex items-center">
            <div className="bg-[#0369a1] w-10 h-10 rounded-lg flex items-center justify-center mr-3 shadow-sm">
            ...
            {getLinks().map((link) => (
              <li key={link.id}>
                <button
                  ...
                  className={`flex items-center w-full px-5 py-3 rounded-lg transition-all duration-200 ${
                    activeTab === link.id
                ...
                >
                  ...
                  <span className="text-md font-medium tracking-wide">{link.label}</span>
                  {activeTab === link.id && (
        ...
        <div className="px-3 py-5 border-t border-[#0a7ba8]/30">
          <button 
            onClick={handleSignout}
            className="flex items-center w-full px-5 py-3 rounded-lg text-[#d9f1ff] hover:bg-white/5 hover:text-white transition-colors group"
          >
            <FaSignOutAlt className="mr-4 text-[#9fd4ff] group-hover:text-white transition-colors" />
            <span className="text-md font-medium tracking-wide">Sign Out</span>
          </button>
        </div>
      </div>
    );
